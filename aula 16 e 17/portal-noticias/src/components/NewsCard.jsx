...
//     localStorage.getItem(news.url) !== null
//   );
...
//     }}>
//       {news.urlToImage && (
//         <img src={news.urlToImage} alt={news.title} style={{ width: "100%", borderRadius: "8px" }} />
//       )}
//       <h3>{news.title}</h3>
...
export default function NewsCard({ news }) {
  ...
  return (
    <div style={{
      ...
      <small>{news.source?.name}</small>
      <div style={{ marginTop: "0.5rem", display: "flex", justifyContent: "space-between" }}>
        <a href={news.url} target="_blank" rel="noreferrer">Ler mais</a>
        <button onClick={toggleFavorite}>
          {favorited ? "★ Favorito" : "☆ Favoritar"}
        </button>
      </div>
